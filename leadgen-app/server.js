...
// Renders a social icon as a real PNG, on the fly - inline SVG (the
// original approach) has poor/inconsistent support across real email
// clients (Outlook doesn't render it at all; Gmail's support is
// unreliable too), while PNG has near-universal support. Public/no-auth
// since this gets embedded in emails sent to external recipients who
// were never logged into the app.
app.get("/icon/social/:key.png", async (req, res) => {
  try {
    const { renderSocialIconPng } = require("./src/socialIcons");
    const buffer = await renderSocialIconPng(req.params.key, req.query.color || "#666666");
    if (!buffer) return res.status(404).end();
    res.set("Content-Type", "image/png");
    res.set("Cache-Control", "public, max-age=31536000, immutable"); // this exact icon+color combination never changes
    res.send(buffer);
  } catch (err) {
    console.error("[social-icon] Render failed:", err.message);
    res.status(500).end();
  }
});
